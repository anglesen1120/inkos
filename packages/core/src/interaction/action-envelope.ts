...
export function shortRunCharsPerChapterError(value: number, language: "zh" | "en" | "vi"): string {
  ...
  if (language === "zh") {
    return `charsPerChapter=${value} 超出中文短篇的合法范围（每章 ${min}-${max} 个汉字）。` + `charsPerChapter=${value} is outside the valid range for Chinese shorts (${min}-${max} characters per chapter).`;
  }
  if (language === "vi") {
    return `charsPerChapter=${value} vượt ngoài phạm vi hợp lệ cho truyện ngắn tiếng Việt (${min}-${max} từ mỗi chương).` + `charsPerChapter=${value} is outside the valid range for Vietnamese shorts (${min}-${max} words per chapter).`;
  }
  return `charsPerChapter=${value} 超出英文短篇的合法范围（每章 ${min}-${max} 个英文单词）。` + `charsPerChapter=${value} is outside the valid range for English shorts (${min}-${max} words per chapter).`;
}
