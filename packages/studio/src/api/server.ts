...
function normalizeStudioLanguage(value: unknown): StudioLanguage {
  ...
  if (value === "vi" || value === "vi-VN" || value === "vi_VN" || value === "vi_VN.UTF-8") return "vi";
  return "zh";
}
...
interface StudioLocalizedLabel {
  readonly zh: string;
  readonly en: string;
  readonly vi: string;
}

const PIPELINE_STAGES: Record<string, ReadonlyArray<StudioLocalizedLabel>> = {
  writer: [
    ...
    { zh: "生成最终真相文件", en: "Generate final truth files", vi: "Tạo tệp sự thật cuối" },
    { zh: "校验真相文件变更", en: "Validate truth file changes", vi: "Kiểm tra thay đổi tệp sự thật" },
    { zh: "同步记忆索引", en: "Sync memory index", vi: "Đồng bộ chỉ mục ký ức" },
    { zh: "更新章节索引与快照", en: "Update chapter index and snapshot", vi: "Cập nhật chỉ mục chương và snapshot" },
  ],
  architect: [
  ...
  reviser: [
    ...
    { zh: "修订章节", en: "Revise chapter", vi: "Sửa chương" },
...
const AGENT_LABELS: Record<string, StudioLocalizedLabel> = {
  architect: { zh: "建书", en: "Book setup", vi: "Thiết lập sách" },
...
const TOOL_LABELS: Record<string, StudioLocalizedLabel> = {
  read: { zh: "读取文件", en: "Read file", vi: "Đọc tệp" },
  edit: { zh: "编辑文件", en: "Edit file", vi: "Sửa tệp" },
  ...
  continuation_import: { zh: "导入续写", en: "Continuation import", vi: "Nhập bản viết tiếp" },
  ...
  create_narrative_forecast: { zh: "剧情多线推演", en: "Narrative forecast", vi: "Dự báo mạch truyện" },
...
function formatTaskElapsed(ms: number, lang: StudioLanguage): string {
  ...
  if (minutes === 0) return pick(lang, `${seconds} 秒`, `${seconds}s`, `${seconds} 秒`);
  return pick(lang, `${minutes} 分 ${seconds} 秒`, `${minutes}m ${seconds}s`, `${minutes} 分 ${seconds} 秒`);
}
...
function buildRunningTaskContextBlock(task: StudioTaskSnapshot, lang: StudioLanguage): string {
  ...
  return pick(
    ...
    [
      ...
      "Tác vụ chạy độc lập ở chế độ nền và không bị gián đoạn bởi lượt trò chuyện này. Khi người dùng hỏi tiến độ, hãy trả lời trung thực theo thông tin trên. Không khởi chạy thêm tác vụ sản xuất cùng loại và không nói rằng không có tác vụ đang chạy. Công cụ sản xuất tạm thời không khả dụng và sẽ hoạt động lại khi tác vụ hoàn tất.",
    ].join("\n"),
  );
}
...
function nonTextModelMessage(modelId: string, lang: StudioLanguage = "zh"): string {
  return pick(
    ...
    `Model ${modelId} không phù hợp để trò chuyện/viết văn bản. Hãy chọn model văn bản trong bộ chọn model, chẳng hạn gemini-2.5-flash, gemini-2.5-pro hoặc model chat của nhà cung cấp.`,
  );
}
...
function validateAgentActionExecution(args: {
  ...
  if (failedExec) {
    const detail = failedExec.error ?? failedExec.result ?? pick(lang, "未知错误", "unknown error", "未知错误");
    return pick(
      lang,
  ...
  if (
    ...
    return pick(
      ...
      "Model cho biết đã hoàn thành chương tiếp theo nhưng chưa gọi công cụ viết. Hãy thử lại; nếu vẫn lỗi, kiểm tra model có hỗ trợ gọi công cụ không.",
    );
  }
  ...
  if (
    ...
    return pick(
      ...
      "Đã xác nhận tạo sách nhưng model chưa gọi công cụ thiết lập sách. Hãy thử lại; nếu vẫn lỗi, kiểm tra model có hỗ trợ gọi công cụ không.",
    );
  }
  ...
  if (args.requestedIntent === "short_run" && !hasSuccessfulToolExec(args.collectedToolExecs, "short_fiction_run")) {
    return pick(
      ...
      "Đã xác nhận tạo truyện ngắn nhưng model chưa gọi công cụ sản xuất truyện ngắn. Hãy thử lại; nếu vẫn lỗi, kiểm tra model có hỗ trợ gọi công cụ không.",
    );
  }
  ...
  if (args.requestedIntent === "play_start" && !hasSuccessfulToolExec(args.collectedToolExecs, "play_start")) {
    return pick(
      ...
      "Đã xác nhận khởi động thế giới tương tác nhưng model chưa gọi công cụ thế giới tương tác. Hãy thử lại; nếu vẫn lỗi, kiểm tra model có hỗ trợ gọi công cụ không.",
    );
  }
  ...
  if (args.requestedIntent === "generate_cover" && !hasSuccessfulToolExec(args.collectedToolExecs, "generate_cover")) {
    return pick(
      ...
      "Đã xác nhận tạo bìa nhưng model chưa gọi công cụ tạo bìa. Hãy thử lại; nếu vẫn lỗi, kiểm tra model có hỗ trợ gọi công cụ không.",
    );
  }
...
function formatAgentFailure(
  ...
  if (kind === "internal") {
    return {
      ...
      message: pick(lang, `InkOS 内部流程错误：${message}`, `InkOS internal pipeline error: ${message}`, `InkOS 内部流程错误：${message}`),
      status: 500,
    };
  }
...
function toolResultText(result: unknown, lang: StudioLanguage = "zh"): string {
  ...
  return text || pick(lang, "已完成。", "Done.", "已完成。");
}
...
async function executeConfirmedProductionAction(args: {
  ...
  if (args.requestedIntent === "create_book") {
    ...
    const title = requirePayloadText(payload?.title, pick(lang, "确认建书缺少书名，请重新生成确认卡。", "The book creation confirmation is missing a title. Regenerate the confirmation card.", "确认建书缺少书名，请重新生成确认卡。"));
    ...
    if (!direction) throw new ApiError(400, "CONFIRMED_ACTION_PAYLOAD_INCOMPLETE", pick(lang, "确认短篇缺少方向，请重新生成确认卡。", "The short fiction confirmation is missing a direction. Regenerate the confirmation card.", "确认短篇缺少方向，请重新生成确认卡。"));
    ...
    if (!args.bookId) {
      throw new ApiError(400, "BOOK_ID_REQUIRED", pick(lang, "写下一章需要先打开一本书。", "Writing the next chapter requires an active book.", "写下一章需要先打开一本书。"));
    }
    ...
    const title = requirePayloadText(payload?.title, pick(lang, "确认生成封面缺少标题，请重新生成确认卡。", "The cover generation confirmation is missing a title. Regenerate the confirmation card.", "确认生成封面缺少标题，请重新生成确认卡。"));
    ...
    const title = requirePayloadText(payload?.title, pick(lang, "确认创建剧本缺少标题，请重新生成确认卡。", "The script creation confirmation is missing a title. Regenerate the confirmation card.", "确认创建剧本缺少标题，请重新生成确认卡。"));
    ...
    const title = requirePayloadText(payload?.title, pick(lang, "确认创建分镜缺少标题，请重新生成确认卡。", "The storyboard creation confirmation is missing a title. Regenerate the confirmation card.", "确认创建分镜缺少标题，请重新生成确认卡。"));
    ...
    const title = requirePayloadText(payload?.title, pick(lang, "确认创建互动影游缺少标题，请重新生成确认卡。", "The interactive film confirmation is missing a title. Regenerate the confirmation card.", "确认创建互动影游缺少标题，请重新生成确认卡。"));
    ...
    const filePath = requirePayloadText(payload?.filePath, pick(lang, "确认创建翻译项目缺少文件路径，请重新生成确认卡。", "The translation confirmation is missing a file path. Regenerate the confirmation card.", "确认创建翻译项目缺少文件路径，请重新生成确认卡。"));
    const sourceLanguage = requirePayloadText(payload?.sourceLanguage, pick(lang, "确认创建翻译项目缺少源语言，请重新生成确认卡。", "The translation confirmation is missing a source language. Regenerate the confirmation card.", "确认创建翻译项目缺少源语言，请重新生成确认卡。"));
    const targetLanguage = requirePayloadText(payload?.targetLanguage, pick(lang, "确认创建翻译项目缺少目标语言，请重新生成确认卡。", "The translation confirmation is missing a target language. Regenerate the confirmation card.", "确认创建翻译项目缺少目标语言，请重新生成确认卡。"));
    ...
    const title = requirePayloadText(payload?.title, pick(lang, "确认创建同人缺少书名，请补充后重新确认。", "The fanfiction confirmation is missing a title.", "确认创建同人缺少书名，请补充后重新确认。"));
    if (!payload?.sourceText?.trim() && !payload?.sourcePath?.trim()) {
      throw new ApiError(400, "CONFIRMED_ACTION_PAYLOAD_INCOMPLETE", pick(lang, "创建同人需要原作资料或上传文件。", "Fanfiction creation requires source material or an uploaded file.", "创建同人需要原作资料或上传文件。"));
    }
    ...
    const sourcePath = requirePayloadText(payload?.sourcePath, pick(lang, "导入续写需要上传文件或章节目录。", "Continuation import requires an uploaded file or chapter directory.", "导入续写需要上传文件或章节目录。"));
    const targetBookId = payload?.bookId ?? args.bookId ?? undefined;
    if (!targetBookId && !payload?.title?.trim()) {
      throw new ApiError(400, "CONFIRMED_ACTION_PAYLOAD_INCOMPLETE", pick(lang, "导入续写需要选择已有书籍或填写新书名。", "Continuation import requires an existing book or a new title.", "导入续写需要选择已有书籍或填写新书名。"));
    }
    ...
    const title = requirePayloadText(payload?.title, pick(lang, "确认创建番外缺少书名。", "The side-story confirmation is missing a title.", "确认创建番外缺少书名。"));
    const parentBookId = requirePayloadText(payload?.parentBookId ?? args.bookId ?? undefined, pick(lang, "创建番外需要指定正传书籍。", "Side-story creation requires a parent book.", "创建番外需要指定正传书籍。"));
    ...
    const title = requirePayloadText(payload?.title, pick(lang, "确认创建仿写缺少书名。", "The imitation confirmation is missing a title.", "确认创建仿写缺少书名。"));
    const storyIdea = requirePayloadText(payload?.storyIdea, pick(lang, "仿写需要一个原创故事方向。", "Style imitation requires an original story idea.", "仿写需要一个原创故事方向。"));
    if (!payload?.referenceText?.trim() && !payload?.referencePath?.trim()) {
      throw new ApiError(400, "CONFIRMED_ACTION_PAYLOAD_INCOMPLETE", pick(lang, "仿写需要参考文本或上传文件。", "Style imitation requires reference text or an uploaded file.", "仿写需要参考文本或上传文件。"));
    }
    ...
    const title = requirePayloadText(payload?.title, pick(lang, "确认启动互动世界缺少标题，请重新生成确认卡。", "The interactive world start confirmation is missing a title. Regenerate the confirmation card.", "确认启动互动世界缺少标题，请重新生成确认卡。"));
    ...
    if (!payload?.node) {
      throw new ApiError(400, "CONFIRMED_ACTION_PAYLOAD_INCOMPLETE", pick(lang, "确认连接选择缺少节点数据，请重新生成确认卡。", "The connect-choice confirmation is missing node data. Regenerate the confirmation card.", "确认连接选择缺少节点数据，请重新生成确认卡。"));
    }
    const projectId = payload?.projectId ?? args.bookId;
    if (!projectId) throw new ApiError(400, "INVALID_ID", "interactive-film action requires a project id (bookId)");
    ...
    if (!payload?.nodeId) {
      throw new ApiError(400, "CONFIRMED_ACTION_PAYLOAD_INCOMPLETE", pick(lang, "确认删除节点缺少 nodeId，请重新生成确认卡。", "The remove-node confirmation is missing a nodeId. Regenerate the confirmation card.", "确认删除节点缺少 nodeId，请重新生成确认卡。"));
    }
    const projectId = payload?.projectId ?? args.bookId;
    if (!projectId) throw new ApiError(400, "INVALID_ID", "interactive-film action requires a project id (bookId)");
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timeout = setTimeout(
          () => reject(new Error(pick(lang, `${label} 超时（${timeoutMs}ms）`, `${label} timed out (${timeoutMs}ms)`, `${label} 超时（${timeoutMs}ms）`))),
          timeoutMs,
        );
      }),
    ]);
...
function formatServiceProbeError(args: {
  ...
  const streamSuffix = typeof args.stream === "boolean"
    ? pick(lang, `，${args.stream ? "流式" : "非流式"}`, `, ${args.stream ? "streaming" : "non-streaming"}`, `，${args.stream ? "trực tuyến" : "không trực tuyến"}`)
    : "";
  const context = [
    pick(lang, `服务商：${args.label ?? args.service}`, `Service: ${args.label ?? args.service}`, `Nhà cung cấp: ${args.label ?? args.service}`),
  ...
  const upstreamPrefix = (detail: string): string =>
    pick(lang, `\n上游返回：${detail}`, `\nUpstream response: ${detail}`, `\nPhản hồi từ upstream: ${detail}`);

  if (args.service === "google") {
    return [
      pick(lang, "Google Gemini 测试连接失败。", "Google Gemini connection test failed.", "Google Gemini 测试连接失败。"),
      context,
      "",
      pick(lang, "请优先检查：", "Check these first:", "请优先检查："),
      pick(
        lang,
        ...
        "1. API Key có phải là Gemini API key từ Google AI Studio, không phải thông tin OAuth, Vertex AI hoặc dịch vụ Google khác.",
      ),
      pick(
        lang,
        ...
        "2. Dự án của key đã bật Gemini API và key không bị giới hạn vào API, nguồn hoặc dịch vụ khác.",
      ),
      pick(
        lang,
      ...
      pick(
        ...
        "4. Nếu key từng bị lộ, hãy tạo lại key trong AI Studio trước khi lưu.",
      ),
  ...
  if (args.service === "moonshot" || args.service === "kimiCodingPlan" || args.service === "kimicode") {
    return [
      ...
      pick(
        ...
        "Trước hết hãy kiểm tra model có khả dụng không và các model như kimi-k2.x có yêu cầu temperature=1 không.",
      ),
  ...
  return [
    ...
    pick(
      ...
      "Hãy kiểm tra API Key, model khả dụng, hạn mức tài khoản và loại giao thức có phù hợp nhà cung cấp không.",
    ),
    rawDetail ? upstreamPrefix(rawDetail) : "",
  ].filter(Boolean).join("\n");
}
...
function buildBearerAuthHeaders(apiKey: string | undefined, lang: StudioLanguage = "zh"): Record<string, string> {
  ...
  if (!/^[\x20-\x7e]+$/.test(trimmed)) {
    throw new Error(pick(
      ...
      "API Key chỉ được chứa chữ cái, chữ số và ký hiệu ASCII thông dụng. Hãy kiểm tra bạn có dán nhầm phần giải thích không.",
    ));
  }
    ...
    if (!discoveredFirstModel || !isTextChatModelId(discoveredFirstModel)) {
      return {
        ...
        error: pick(
          ...
          "Có thể truy cập danh sách model nhưng không tìm thấy model dùng được cho trò chuyện văn bản.",
        ),
      };
    }
    return {
  ...
  if (modelCandidates.length === 0) {
    return {
      ...
      error: pick(
        ...
        "Không thể tự xác định model. Hãy điền model khả dụng hoặc cung cấp endpoint hỗ trợ /models.",
      ),
    };
  }

  let lastError = modelsResponse.error ?? pick(lang, "自动探测失败", "Automatic probing failed", "自动探测失败");
...
export function createStudioServer(initialConfig: ProjectConfig, root: string, overrides: { readonly nodeImageGenerator?: NodeImageDeps } = {}) {
    ...
    const reconciled: StudioTaskSnapshot = {
      ...
      execution: {
        ...
        error: pick(
          ...
          "Tác vụ bị gián đoạn: máy chủ Studio khởi động lại khi tác vụ đang chạy. Vui lòng thực hiện lại.",
        ),
        completedAt,
      },
    };
  ...
  app.post("/api/v1/books/create", async (c) => {
    ...
    processProjectInteractionRequest({
      ...
      request: {
        ...
        title: body.title,
        genre: body.genre,
        language: body.language === "en" ? "en" : body.language === "vi" ? "vi" : body.language === "zh" ? "zh" : undefined,
        ...
        [
          {
            ...
            content: language === "en"
              ? [
                  "You help writers refresh a single chapter brief.",
                  "Given the current chapter, persisted brief, and generated plan, suggest one concrete alternative scene, clue, action detail, or chapter-ending turn.",
                  "Do not write the whole chapter, do not alter established facts, and do not claim files were modified.",
                  "Return a concise, readable Markdown inspiration card only.",
                ].join("\n")
              : language === "vi"
                ? [
                    "Bạn hỗ trợ tác giả làm mới brief cho một chương.",
                    "Dựa trên chương hiện tại, brief đã lưu và kế hoạch chương do hệ thống tạo, hãy đề xuất một cảnh thay thế, manh mối, chi tiết hành động hoặc cú ngoặt cuối chương thật cụ thể.",
                    "Không viết thay toàn bộ chương, không thay đổi sự kiện đã xác lập, và không nói rằng file đã được chỉnh sửa.",
                    "Chỉ trả về một thẻ gợi ý Markdown ngắn gọn, dễ đọc.",
                  ].join("\n")
                : [
                    "你帮助作者刷新单章提示词。",
                    "根据当前章节、已保存提示词和系统章节计划，给出一个符合现有设定的具体替代场面、证据或行动细节，以及章尾转折。",
          ...
          },
          {
            ...
            content: [
              ...
              requestedBrief || persistedBrief
                ? `${language === "en" ? "Current user brief" : language === "vi" ? "Brief hiện tại của người dùng" : "当前用户提示"}:\n${requestedBrief || persistedBrief}`
                : "",
              plan ? `${language === "en" ? "Generated chapter plan" : language === "vi" ? "Kế hoạch chương đã tạo" : "系统章节计划"}:\n${plan}` : "",
    ...
    if (!env || !env.values.apiKey) {
      return c.json({
        error: pick(
          ...
          "Không phát hiện cấu hình biến môi trường LLM để nhập hoặc đang thiếu INKOS_LLM_API_KEY.",
        ),
      }, 400);
    }
    ...
    if (body.configSource === "env") {
      return c.json({
        error: pick(
          ...
          "Runtime Studio không hỗ trợ chuyển sang env; env chỉ là lớp ghi đè trong CLI/daemon/runtime triển khai.",
        ),
      }, 400);
    }
    ...
    if (requestedBaseUrl && !baseUrl) {
      return c.json({
        error: pick(
          ...
          "Base URL bìa phải là địa chỉ HTTP(S) hợp lệ, không chứa thông tin đăng nhập, tham số truy vấn hoặc fragment.",
        ),
      }, 400);
    }
    ...
    if (trimmedKey && !isHeaderSafeApiKey(trimmedKey)) {
      return c.json({
        error: pick(
          ...
          "API Key chứa ký tự không thể đặt trong HTTP Authorization header. Hãy chỉ dán khóa gốc.",
        ),
      }, 400);
    }
  ...
  app.post("/api/v1/services/:service/test", async (c) => {
    ...
    if (!resolvedBaseUrl) {
      return c.json({
        ok: false,
        error: pick(language, `未知服务商: ${service}`, `Unknown service: ${service}`, `未知服务商: ${service}`),
      }, 400);
    }
    ...
    if (!apiKey?.trim() && !apiKeyOptional) {
      return c.json({
        ok: false,
        error: pick(language, "API Key 不能为空", "API Key must not be empty", "API Key 不能为空"),
      }, 400);
    }
    ...
    const connectionFailed = pick(language, "连接失败", "Connection failed", "连接失败");
    const probeStatus = {
      ok: probe.ok,
      ...
      if (!isHeaderSafeApiKey(trimmedKey)) {
        return c.json({
          ...
          error: pick(
            ...
            "API Key chỉ được chứa ký tự ASCII không trắng phù hợp với HTTP Authorization header; không dán thông báo lỗi kết nối hoặc văn bản chẩn đoán.",
          ),
        }, 400);
      }
  ...
  app.put("/api/v1/project", async (c) => {
    ...
    try {
      ...
      if (updates.language !== undefined) {
        existing.language = normalizeStudioLanguage(updates.language);
      }
    ...
    try {
          ...
          if (/API key/i.test(msg)) {
            return c.json({
              ...
              response: pick(
                ...
                `Trước hết hãy điền API Key cho ${reqService} trong cài đặt model rồi thử lại.`,
              ),
            }, 400);
          }
          throw e;
      ...
      if (confirmedIntent && confirmedTaskId) {
        const productionTaskBusyResponse = () => {
          const message = pick(
            ...
            "Phiên này đang có tác vụ sản xuất chạy. Hãy đợi hoàn tất hoặc dừng tác vụ trước khi tạo tác vụ mới.",
          );
          return c.json({
        ...
        try {
          ...
          if (exec.status === "completed") {
            ...
            if (createdBookId) {
              if (!await completeBookExists(join(root, "books", createdBookId))) {
                const message = pick(surfaceLanguage, "创作工具返回了建书结果，但磁盘上的书籍工件不完整。", "The creation tool returned a book result, but the on-disk book artifact is incomplete.", "创作工具返回了建书结果，但磁盘上的书籍工件不完整。");
          ...
          const responseText = exec.result ?? pick(surfaceLanguage, "已完成。", "Done.", "已完成。");
      ...
      if (!result.responseText) {
        ...
        const emptyMessage = pick(
          ...
          "Model không trả về nội dung văn bản. Hãy kiểm tra loại giao thức (chat/responses), chế độ streaming hoặc khả năng tương thích của dịch vụ nguồn.",
        );
        if (resolveCreatedBookIdFromToolExecs(collectedToolExecs)) {
          await finalizeCreatedBook();
        }
      ...
      if (/already processing|prompt.*queue/i.test(msg)) {
        return c.json({
          error: {
            code: "AGENT_BUSY",
            message: pick(language, "正在处理中，请等待当前操作完成", "Still processing. Wait for the current operation to finish", "Đang xử lý. Hãy đợi thao tác hiện tại hoàn tất"),
          },
          response: pick(
            language,
            "正在处理中，请等待当前操作完成后再发送。",
            "Still processing. Wait for the current operation to finish before sending again.",
            "Đang xử lý. Hãy đợi thao tác hiện tại hoàn tất rồi gửi lại.",
          ),
        }, 429);
      }
  ...
  app.post("/api/v1/project/language", async (c) => {
    ...
    try {
      ...
      await writeFile(configPath, JSON.stringify(existing, null, 2), "utf-8");
      return c.json({ ok: true, language: normalized });
    } catch (e) {
      return c.json({ error: String(e) }, 500);
    }
  });
